...
class InternalProcessing(Document):
	def on_submit(self):
		...
		# 		data_doc.parentfield="table_115"
		# 		data_doc.stages=self.doctype
...
@frappe.whitelist()
def make_maintenance_invoice(doc):
...
def update_all_document(doc,doctype,status):
    if doc:
        ...
        frappe.db.set_value("Maintenance Directing",m_direct.maintenance_directing,{"status":status})
        m_order=frappe.get_doc("Maintenance Directing",m_direct.maintenance_directing)
        frappe.db.set_value("Maintenance Order",m_order.maintenance_order,{"status":status})
