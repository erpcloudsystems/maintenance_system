    ...
    show_data: function (frm) {
        let d = new frappe.ui.Dialog({
            ...
            fields: [
                ...
                // 	fieldname: 'm_total',
                // 	fieldtype: 'Currency',
                // 	default: frm.doc.material_total,
                // 	read_only: 1
                // },
                // {
                // 	fieldname: 'cb_2',
        ...
        if (frm.doc.docstatus < 1) {
            frappe.call({
                ...
                callback: function (r) {
                    ...
                    if (r.message.name) {
                        cur_frm.set_value("confirmed_by", r.message.name);
                    }
                    // else{
                    //     frappe.throw("Maintenance Team User Not Found")
                    // }
                    cur_frm.set_value("current_stage", cur_frm.doctype)
                }
            })
        }
    }
    ...
    onload(frm) {
        if (!frm.doc.price_list && frm.doc.docstatus == 0) {
            frappe.model.with_doc("Maintenance System Settings", frm.doc.trigger, function () {
                ...
                frm.set_value("company", tabletransfer.comapny)
        ...
        frappe.model.with_doc("Sales Taxes and Charges Template", frm.doc.sales_taxes_and_charges_template, function () {
            ...
            $.each(tabletransfer.taxes, function (index, row) {
                ...
                if (typeof frm.doc.total_amount !== "undefined") {
                    ...
                    // frm.set_value("outstanding_amount",frm.doc.total_amount+service_t+frm.doc.total_taxes_and_charges)
                } else {
                    frm.set_value("grand_total", service_t + frm.doc.total_taxes_and_charges)
                    // frm.set_value("outstanding_amount",service_t+frm.doc.total_taxes_and_charges)
                }
        ...
        frappe.model.with_doc("Sales Taxes and Charges Template", frm.doc.sales_taxes_and_charges_template, function () {
            ...
            $.each(tabletransfer.taxes, function (index, row) {
                ...
                if (typeof frm.doc.total_amount !== "undefined") {
                    ...
                    // frm.set_value("outstanding_amount",frm.doc.total_amount+frm.doc.service_total+taxes)
                } else {
                    frm.set_value("grand_total", frm.doc.service_total + taxes)
                    // frm.set_value("outstanding_amount",frm.doc.service_total+taxes)
                }
                refresh_field("grand_total");
                // refresh_field("outstanding_amount");
                cur_frm.refresh_field("tax");
...
frappe.ui.form.on("Maintenance Invoice", "warranty_apply", function (frm) {
    ...
    if (frm.doc.warranty_apply == 1) {
        ...
        if (frm.doc.warranty_bearing_rate.length >= 1) {
            ...
            var percentage = parseFloat(100) - warranty_bearing_percentage;
...
frappe.ui.form.on("Maintenance Invoice", "warranty_bearing_percentage", function (frm) {
