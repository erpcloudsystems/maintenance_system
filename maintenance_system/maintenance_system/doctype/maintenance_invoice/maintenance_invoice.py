...
@frappe.whitelist()
def make_payment_entry_invoice(doc,mode):
	...
	"paid_from": paid_from,
	"paid_to": paid_to,
	"paid_amount": mi.grand_total,
	"total_allocated_amount" : mi.grand_total,
	"unallocated_amount": 0.0,
	"references": [{
		"reference_doctype": "Maintenance Invoice",
		"reference_name": mi.name,
		...
		"allocated_amount": mi.grand_total,
		"pending" : 0.0
		}]
	})
	pe.insert(ignore_permissions=True)
	pe.save(ignore_permissions=True)
	pe.submit()
	frappe.db.set_value("Maintenance Invoice",doc,"payment_received",1)
	if mi.grand_total < pe.paid_amount:
		frappe.db.set_value("Maintenance Invoice",doc,"paid_amount",pe.paid_amount-mi.grand_total)
	elif mi.grand_total > pe.paid_amount:
		frappe.db.set_value("Maintenance Invoice",doc,"outstanding_amount",mi.grand_total-pe.paid_amount)
	else:
		frappe.db.set_value("Maintenance Invoice",doc,"outstanding_amount",0)
		frappe.db.set_value("Maintenance Invoice",doc,"paid_amount",pe.paid_amount)
