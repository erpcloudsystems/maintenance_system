...
class MaintenanceOrder(Document):
	def validate(self):
		maintenance_bearing(self)
		if not len(self.maintenance_malfunctions) > 0:
			frappe.throw("Please Enter Maintenance Malfunctions")
		elif not len(self.table_60) > 0:
			frappe.throw("Please Enter Maintenance Service")
		self.get_company()
	...
	def get_company(self):
		if self:
			...
			if get_company.comapny:
				# company_address=frappe.db.get_value("Dynamic Link",{"link_name":get_company.comapny,"link_doctype":"Company"},"parent")
				# self.company_address=company_address
				self.terms_and_conditions=get_company.terms_of_service
				if get_company.terms_of_service:
					get_terms=frappe.get_doc("Terms and Conditions",get_company.terms_of_service)
					self.terms=get_terms.terms
				self.company=get_company.comapny
			else:
				frappe.throw("Please Set a Default Company in Maintenance System Settings")

	def on_submit(self):
		if self.status == "Waiting":
			items = []
			for d in self.maintenance_malfunctions:
				item_li = {"malfunction": d.malfunction,"customer_note":d.customer_note,"maintenance_notes":d.maintenance_notes}
			...
			self.save()
			frappe.clear_cache()
			frappe.db.set_value(self.doctype,self.name,"maintenance_directing",so.name)
			self.reload()


		if self.receive_maintenance_items:
			sv = frappe.get_doc("Receive Maintenance Item",self.receive_maintenance_items)
			sv.status = "Occupied"
		...
		if self.commission_benificiary:		
			...
			data_doc.parentfield="table_90"
			data_doc.stages=self.doctype
			data_doc.team=self.confirmed_by
			data_doc.insert()
			frappe.clear_cache()
			self.reload()
...
def get_branch(branch):
	if branch:
		maintenance_department= frappe.db.sql(f"""select parent from `tabMaintenance Department Branch` where branch = '{branch}' """,as_list=1)
...
@frappe.whitelist()
def default_branch():
    branch=frappe.db.get_value("Branch",{"default_branch":1,"enable":1},"name")
    return branch

@frappe.whitelist()
def order_default_branch(processing,doctype):
    if processing:
        m_order=frappe.db.get_value(doctype,{"name":processing},"maintenance_order")
        if m_order:
            branch=frappe.db.get_value("Maintenance Order",{"name":m_order},"branch")
            if branch:
                return branch
...
def get_company_address(company):
	if company:
		address=frappe.db.get_value("Dynamic Link",{"link_doctype":"Company","link_name":company},"parent")
		return address