...
frappe.ui.form.on('Maintenance Order', {
    validate: function (frm) {
        if (frm.doc.warranty_start_date && frm.doc.warranty_period) {
            ...
            console.log(ny + "-" + nm + "-" + ndate);
            var date1 = ny + "-" + nm + "-" + ndate;
            frm.set_value("warranty_expiry_date", date1);
        }
        frm.trigger("company_address")
        if (!frm.doc.confirmed_by) {
            frappe.throw("No Maintenance Team Found for this user")
        }
    },
    company_address(frm) {
        ...
        if (frm.doc.docstatus < 1) {
            frappe.call({
                ...
                callback: function (r) {
                    ...
                    // else{
                    //     frappe.msgprint("Maintenance Team User Not Found")
                    // }
...
frappe.ui.form.on('Maintenance Order', 'onload', function (frm) {
    // frm.set_df_property('maintenance_schedule', 'reqd', 1);

    /*	if(!maintenance_malfunctions){
            frm.set_df_property('maintenance_malfunctions',  'reqd',1);
        }*/
    ...
    onload(frm) {
        if (!frm.doc.price_list && frm.doc.docstatus < 1) {
            frappe.model.with_doc("Maintenance System Settings", frm.doc.trigger, function () {
                ...
                console.log(tabletransfer.comapny)
...
// Update Sales Taxes and Charges 

frappe.ui.form.on("Maintenance Order", {
    refresh: function (frm) {
        if (!frm.doc.sales_taxes_and_charges_template && frm.doc.docstatus < 1) {
            frappe.call({
                method: 'frappe.client.get_value',
                args: {
